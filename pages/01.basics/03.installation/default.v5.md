...
**Note** that version 4.x.x and version 5.x.x cannot coexist in the same folder so if you install both, remember to install them in separate folders.
...
```
Advanced users familiar with dependencies may consider using `sudo dpkg -i qlcplus_x.y.z.deb`.
