...
If you need fade in/out automations together with the control of the Function intensity, then the usage of a ![](/basics/button.png)[Virtual Console Button](vcbutton.html) in combination with a Slider in Submaster mode is what you're looking for.
...
For example a submaster can control the intensity of a function attached to a [button](vcbutton.html) or the channel levels associated to a slider in level mode.  
